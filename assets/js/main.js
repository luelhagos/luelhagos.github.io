...
(function() {
  ...
  /**
   * Activate work tab from nav links with data-activate-tab attribute
   */
  document.querySelectorAll('[data-activate-tab]').forEach(link => {
    link.addEventListener('click', function() {
      const tabId = this.getAttribute('data-activate-tab');
      const tab = document.getElementById(tabId);
      if (tab) {
        setTimeout(() => new bootstrap.Tab(tab).show(), 200);
      }
    });
  });
